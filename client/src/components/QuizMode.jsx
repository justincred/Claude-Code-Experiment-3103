...
function QuizMode({ documentId, onBack }) {
  ...
  const fetchNextQuestion = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/study/next-question/${documentId}`);
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      setSessionStats(prev => ({
        ...
        avgScore: ((prev.avgScore * prev.questionsAnswered + response.data.score) / (prev.questionsAnswered + 1))
      }));
        ...
        <div className="empty-state">
          ...
          <div className="final-stats">
            ...
            <div className="stat-box">
              ...
              <div className="stat-value">{stats.reviewed > 0 ? Math.round((stats.correct / stats.reviewed) * 100) : 0}%</div>
            </div>
          </div>
    ...
    <div className="quiz-mode">
      <div className="quiz-header">
        ...
        <div className="quiz-stats">
          <span>📊 Concept: <strong>{currentQuestion.conceptTitle}</strong></span>
          <span>✓ {stats.correct}/{stats.reviewed}</span>
        </div>
      </div>
      ...
      <div className="quiz-container">
        <div className="question-card">
          <div className="progress-bar">
            <div
              ...
              style={{
                width: `${sessionStats.questionsAnswered > 0 ? (sessionStats.correct / sessionStats.questionsAnswered) * 100 : 0}%`
              }}
            ></div>
          </div>
          ...
          {showAnswer && (
            <div className={`evaluation ${evaluation.isCorrect ? 'correct' : 'incorrect'}`}>
              ...
              <div className="score-box">
                ...
                <span className={`score-value ${evaluation.isCorrect ? 'correct' : 'incorrect'}`}>
                  {evaluation.score}%
                </span>
              </div>
        ...
        <div className="quiz-sidebar">
          <div className="sidebar-section">
            ...
            <div className="stat-item">
              <span>Avg Score</span>
              <strong>{Math.round(sessionStats.avgScore)}%</strong>
            </div>
          </div>
